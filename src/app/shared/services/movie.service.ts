...
export class MovieService {
  ...
  public getMovieById(IdMovie: number): Observable<any> {
    return this.httpClient.get(`${this.apiURL}/api/Movies/IdFilme=${IdMovie}`);
  }

  public getMoviePlaces(IdMovie: number, IdSala: number): Observable<any> {
    return this.httpClient.get(`${this.apiURL}/api/ChoosePlace?IdSala=${IdSala}&IdFilme=${IdMovie}`);
  }
